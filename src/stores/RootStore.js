...
import UIStore from './UIStore'; 
 ...
 class RootStore{
    ...
    sendPost(url,_params,noLoading){
        ...
        return this.agent.post(url,params,noLoading).then(json=>this._handleData(json,url,params))
    }
    /**
     * 发生GET请求
     * @param {*请求地址} url 
    ...
    sendGet(url,_params,noLoading){
        const params=this._buildParams(_params);
        return this.agent.get(url,params,noLoading).then(json=>this._handleData(json,url,params))
    }
    /**
     * 显示提示信息，默认3秒
     * @param {*提示内容} msg 
